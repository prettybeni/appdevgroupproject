...
export default {
  data() {
    return {
      product: {},
      quantity: 1,
      sugar_level: 100,  // Default sugar level to 100%
      size: 'Small',     // Default size to Small
      note: '',
      errorMessage: '',
      token: '',
    };
  ...
  try {
    const response = await axios.post(
      `http://localhost:5000/api/cart/${this.product.id}?token=${this.$route.query.token}`, // Adjusted URL
      {
    ...
    if (response.status === 200) {
      this.$router.push(`/menu?token=${this.$route.query.token}`);
    }
  } catch (error) {
    console.error(error);
    // Check if the error response exists and provide a more detailed message
    if (error.response && error.response.data) {
      this.errorMessage = error.response.data.message || 'Failed to add item to cart';
    } else {
